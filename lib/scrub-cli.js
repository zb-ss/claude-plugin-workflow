...
// Neutral public IPs explicitly allowed in tests/docs. Operators can add
// project-specific placeholder IPs via the denylist file's optional
// "allow_ips" array (see loadDenylist) rather than editing this constant.
...
function loadDenylist(denylistPath) {
  ...
  if (!p) return { loaded: false, entries: [], allowIps: new Set() };
  try {
    ...
    // Optional exact-match IP allowlist: un-flags specific public IPs (e.g.
    // long-standing test placeholders) WITHOUT weakening secret / denylist /
    // other-IP detection. Exact string match only — never a pattern.
    const allowIps = new Set(raw.allow_ips || []);
    return { loaded: true, entries, allowIps };
  } catch {
    return { loaded: false, entries: [], allowIps: new Set() };
  }
}
...
function scanText(text, where, denylist, allowIps) {
  const out = [];
  ...
  while ((ipMatch = IPV4_RE.exec(text)) !== null) {
    ...
    if (allowIps && allowIps.has(ip)) continue;
...
function scanSurface(surface, opts) {
  ...
  const allowIps = dl.allowIps;
  hits.push(...scanText(s.branch, 'branch', dl.entries, allowIps));
  for (const c of (s.commits || [])) hits.push(...scanText(c, 'commit', dl.entries, allowIps));
  hits.push(...scanText(s.diff, 'diff', dl.entries, allowIps));
  hits.push(...scanText(s.pr_title, 'pr_title', dl.entries, allowIps));
  hits.push(...scanText(s.pr_body, 'pr_body', dl.entries, allowIps));
