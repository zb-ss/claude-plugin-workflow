...
function buildGhArgs(op, params) {
  ...
  switch (op) {
    case 'list': {
      // gh issue list --repo <repo> [--label <l>] --state open --json ...
      // No default label: listQueued fetches ALL open issues (direct issues API,
      // which is lag-free) and filters by label in JS. A `--label`-filtered gh
      // query goes through search indexing, which lags for seconds after a label
      // change and would intermittently miss a freshly-enqueued task.
      const args = ['issue', 'list', '--repo', p.repo];
      if (p.label) args.push('--label', p.label);
      args.push('--state', 'open', '--json', 'number,title,body,labels');
      return args;
    }
...
/** List all open issues labelled 'queued' (lag-free: fetch open, filter in JS). */
function listQueued(repo) {
  const open = gh(buildGhArgs('list', { repo }));
  return open.filter(i => (i.labels || []).some(l => ((l && l.name) || l) === LABELS.QUEUED));
}
  ...
  try {
    switch (cmd) {
      ...
      case 'readTask': {
        ...
        const issue = readTask(repo, n);
        // Return the PARSED task spec (what the driver needs), plus issue metadata.
        const spec = parseTaskSpec(issue.body || '');
        const result = {
          number: issue.number,
          title: issue.title,
          labels: (issue.labels || []).map(l => (l && l.name) || l),
          ...spec,
        };
        process.stdout.write(JSON.stringify(result, null, 2) + '\n');
        break;
