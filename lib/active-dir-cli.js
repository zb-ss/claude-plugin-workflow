...
if (flags.has('--all')) {
  // Includes repo_key / repo_root so creation can stamp self-describing state
  // in one call — no divergent bash needed.
  process.stdout.write(JSON.stringify({
    state_root: getStateDir(),
    active_base: getActiveBaseDir(),
    active: activeDir,
    repo_key: getRepoKey(cwd),
    repo_root: getRepoRoot(cwd),
  }) + '\n');
} else {
