 ...
 * other repo never confuses the driver. Tasks `parked` awaiting a human (blocked
 * on low confidence / spend cap / parse error) are also excluded so they release
 * the in-flight slot and the next queued task can proceed — a rate-limited task,
 * by contrast, is NOT parked: it keeps the slot and auto-resumes after the reset.
  ...
  for (const d of subdirs) {
    ...
    for (const f of files) {
      ...
      try {
        ...
        const parked = state && state.parked; // blocked, awaiting a human → released the slot
        if (issue !== undefined && issue !== null && issue !== '' && !parked) {
